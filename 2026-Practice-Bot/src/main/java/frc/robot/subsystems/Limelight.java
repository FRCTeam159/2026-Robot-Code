...
public class Limelight extends SubsystemBase {
    ...
    private DoubleSubscriber tv_sub = limelight_table.getDoubleTopic("tv").subscribe(0.0);
    ...
    public void periodic(){
        double tx = tx_sub.get();
        double tv = tv_sub.get();
        ...
        // double distance_in = 317.5 * Math.pow(ty, -0.5935); //inches
        // double distance = distance_in * 0.0254; //meters

        //area the april tag takes on the screen
        double ta = ta_sub.get();
